...
public class IPadPro {
    ...
    boolean continueOrder() {
        ...
        System.out.println("구입하시려면 y / 종료는 q : ");
        String isProduct = sc.next();
        if(isProduct.equalsIgnoreCase("y")) return true;
        else return false;
    }
    ...
    void setScreen( ) {
        Scanner sc = new Scanner(System.in);
        while(true) {
            System.out.print("디스플레이 크기 선택 [1] 11인치, [2] 12.9인치 : ");
            screen = sc.nextInt();
            if (screen == 1 || screen == 2) return;
            System.out.println("크기를 다시 선택하세요.");
        }
    }
    void setColor() {
        Scanner sc = new Scanner((System.in));
    ...
    void setName( ) {
        Scanner sc = new Scanner(System.in);
        System.out.print("각인 서비스를 신청 하시겠습니까?(y/n) : ");
        String service = sc.next();
        if (service.equalsIgnoreCase("Y")) {
            System.out.print("각인 문구를 입력하세요 : ");
            name = sc.next();
        }
    }
    /*
